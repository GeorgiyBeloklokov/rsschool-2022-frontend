...
export default class ApiWinner {
  ...
  public async getWinners({
    ...
    const response = await fetch(
      `${winners}?_page=${page}&_limit=${limit}${this.getSortOrder({ sort, order })}`,
    );
    ...
    return {
      items: await Promise.all(
        items.map(async (winner: IWinner) => ({
          ...winner,
          car: await ApiGarage.getCar(winner.id),
        })),
      ),
  ...
  public async getWinner(id: string): Promise<IWinner> {
    return (await fetch(`${winners}/${id}`)).json();
  }

  public async getWinnerStatus(id: string): Promise<number> {
  ...
  async updateWinner(id: string, body: IWinner): Promise<void> {
    (
      await fetch(`${winners}/${id}`, {
        method: 'PUT',
        body: JSON.stringify(body),
  ...
  async saveWinner({ id, time }: { id: string; time: number }): Promise<void> {
