...
let score = null
//Проверка памяти
if(localStorage.getItem('BirdSongCurLanguage')){
    curLangRus = JSON.parse(localStorage.getItem('BirdSongCurLanguage'))
    changeLangFunc()
}
//Получение счета
function getScore(lang){
  score = JSON.parse(localStorage.getItem('BirdSongScore'))
  if(lang){
    if(localStorage.getItem('BirdSongScore')){
      if(score==30){
...
changeLang.addEventListener('click',()=>{
  ...
  changeLangFunc()
  localStorage.setItem('BirdSongCurLanguage',JSON.stringify(curLangRus))
})
...
mainBtn.addEventListener('click',()=>{
  ...
  localStorage.removeItem('BirdSongScore')
})
