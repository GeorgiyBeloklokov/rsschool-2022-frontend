...
export default class Race {
  ...
  private async raceAll(
    promises: Promise<IDrivingParam>[],
    ids: string[],
  ): Promise<IWinnerParam> {
    const { success, id, time } = await Promise.race(promises);
    if (!success) {
      const failedIndex = ids.findIndex((i: string) => i === id);
      /// console.log(`i =>>>>>>>`, i);

      const promisesWithoutFailed = [
      ...
      return this.raceAll(promisesWithoutFailed, idsWithoutFailed);
    }
